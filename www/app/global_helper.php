...
if (!function_exists('cstr_data_path')) {
    function cstr_data_path($path){
...
if (!function_exists('cstr_url')) {
    function cstr_url($path = "", array $query = []){
        $url = cstr_config('app.url');
...
if (!function_exists('cstr_config')) {
    function cstr_config(string $key = null, $value = null) {
        $args = func_get_args();
        try {
...
if (!function_exists('cstr_trans')) {
    function cstr_trans(string $string){
        $t = 'castoro.'.$string;
...
if (!function_exists('app_url')) {
    function app_url($path = "", array $query = []){
        ...
        if(!empty(cstr_config('external_url')))
            $url = cstr_config('external_url');
...
if (!function_exists('jellyfin_response')) {
    function jellyfin_response($request){
        ...
        $query = array_merge($request->query(), ['cstrCall' => true]);
...
if (!function_exists('jellyfin_url')) {
    function jellyfin_url($path = "", array $query = []){
        $url = cstr_config('jellyfin.external_url');
        $path = !str_starts_with($path, '/') ? '/' . $path : $path;
        $query = array_merge($query, ['cstrCall' => true]);
...
if (!function_exists('get_item_path')) {
    function get_item_path(string $itemPath){
        try {
            $path = str_replace(cstr_data_path(''), '', $itemPath);
