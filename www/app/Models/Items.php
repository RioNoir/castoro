...
class Items extends Model
{
    ...
    public function getTitleData($forceOnline = false){
        ...
        if (isset($this->item_path) && !$forceOnline) {
            ...
            $json = cstr_data_path($this->item_path . '/' . $metaId . '.json');
            if(!file_exists($json))
    ...
    public function removeFromLibrary($removeFolder = false): bool {
        if($removeFolder && isset($this->item_path) && file_exists(cstr_data_path($this->item_path))) {
            try{
                remove_dir(cstr_data_path($this->item_path));
            }catch (\Exception $e){}
        }
        $this->item_path = null;
...
//        $overview .= "- Enjoy.\n";
//        $overview .= "------------------------------\n\n";
        $overview = cstr_trans('add_to_library_desc')."\n\n";
        ...
        $outcome['ServerId'] = cstr_config('server_id');
    ...
    public function getJellyfinListItem($type = "CollectionFolder"){
        ...
        $outcome = array_merge($outcome, [
            ...
            'ServerId' => cstr_config('server_id'),
