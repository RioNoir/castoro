...
class JellyfinController extends Controller
{
            ...
            if(isset($item)) {
                if(in_array($item['Type'], ['Movie', 'Series'])){
                    $metadata['ExternalIdInfos'][] = [
                        'Name' => 'Castoro',
                        'Key' => 'JP',
                        'UrlFormatString' => app_url('/web/#/configurationpage?name=CSTR_ITEM&itemId={0}'),
                    ];
                }
                ...
                if(in_array($item['Type'], ['Movie', 'Series', 'Season', 'Episode'])){
                    $metadata['ExternalIdInfos'][] = [
                        'Name' => 'Castoro Stream (Direct Url)',
                        'Key' => 'JPStreamUrl',
                        'UrlFormatString' => '{0}',
                    ];
                }
    ...
    public function getAuthKeys(Request $request) {
        ...
        $response['Items'] = array_values(array_filter(array_map(function ($item) {
            return $item['AppName'] !== cstr_config('app.code_name') ? $item : null;
        }, $response['Items'])));
        return $this->response->setBody($response)->setStatus(200)->getResponse();
    }
    ...
    public function postAuthKeys(Request $request) {
        $response = [];
        if($request->get('App') !== cstr_config('app.code_name'))
            $response = $this->response->make()->getBody(true);
        return $this->response->setBody($response)->setStatus(200)->getResponse();
    }
    ...
    public function deleteAuthKey(string $accessToken, Request $request) {
        $response = [];
        if($accessToken !== cstr_config('api_key'))
            $response = $this->response->make()->getBody(true);
        return $this->response->setBody($response)->setStatus(200)->getResponse();
    }
...
//            'name' => 'Discover',
//            'icon' => 'search',
//            'url' => '#/list.html?parentId='.md5('_discover').'&serverId='.cstr_config('server_id'),
//        ];
    ...
    public function getWebServerInfo(Request $request) {
        ...
        if(isset($apiKey) && $apiKey == cstr_config('api_key')){
    ...
    public function getSystemInfoPublic(Request $request) {
        ...
        if(!empty($response)){
            ...
            $response['LocalAddress'] = cstr_config('jellyfin.external_url');
            $response['ClientIp'] = $clientIp;
            $response['ClientIpLocal'] = local_ip($clientIp);
            if($response['Id'] !== cstr_config('server_id'))
                cstr_config('server_id', $response['Id']);
        }
        return $this->response->setBody($response)->setStatus(200)->getResponse();
    }
    ...
    public function getSystemLogs(Request $request) {
        ...
        $spLogs = dir_tree(cstr_data_path('app/logs'), true);
    ...
    public function getSystemLog(Request $request) {
        ...
        if(isset($log) && str_starts_with($log, 'castoro')){
            $api = new JellyfinApiManager();
            if($api->testApiKey($apiKey)) {
                $filepath = cstr_data_path('app/logs/' . $log);
    ...
    public function getVirtualFolders(Request $request) {
        ...
        foreach (cstr_config('jellyfin.virtual_folders') as $virtualFolder){
        ...
        system("chown -R ".env('USER_NAME').":".env('USER_NAME')." ".cstr_data_path('/jellyfin'));
    ...
    public function getConfigurationPage(Request $request){
        ...
        }elseif($name == "CSTR_CONF"){
            return view('configuration');
        }
        return $this->response->setBody([])->setStatus(404)->getResponse();
    }

    public function getCastoroConfiguration(Request $request){
        return $this->response->setBody(cstr_config())->setStatus(200)->getResponse();
    }

    public function postCastoroConfiguration(Request $request){
        $api = new JellyfinApiManager();
        ...
        if(!empty($user) && (bool) @$user['Policy']['IsAdministrator']){
            foreach ($request->all() as $key => $value) {
                ...
                $oldMd5 = md5(is_array(cstr_config($key)) ? json_encode(cstr_config($key)) : cstr_config($key));
                if($newMd5 !== $oldMd5)
                    cstr_config($key, $value);
            }
        }
        return $this->response->setBody(cstr_config())->setStatus(200)->getResponse();
    }

    public function getCastoroItem(Request $request){
        return $this->response->setBody([])->setStatus(200)->getResponse();
    }

    public function postCastoroItem(Request $request){
        $api = new JellyfinApiManager();
        $user = $api->getAuthUser();
        if(!empty($user) && (bool) @$user['Policy']['IsAdministrator']){
            $data = $request->all();
    ...
    public function getCastoroDownload(Request $request){
        return $this->response->setBody([])->setStatus(200)->getResponse();
    }

    public function postCastoroDownload(Request $request){
        $api = new JellyfinApiManager();
        $user = $api->getAuthUser();
        if(!empty($user) && (bool) @$user['Policy']['IsAdministrator']){
            $data = $request->all();
