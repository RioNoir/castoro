...
class StreamController extends BaseController
{
    public function getStream(Request $request){
        ...
        $streamResolution = $request->get('streamResolution', cstr_config('stream.resolution'));
        $streamFormat = $request->get('streamFormat', cstr_config('stream.format'));
        $streamCompression = $request->get('streamCompression', cstr_config('stream.compression'));
        $streamQuality = $request->get('streamQuality', cstr_config('stream.quality'));
        $streamAudio = $request->get('streamAudio', cstr_config('stream.audio_format'));
        ...
        $streamCacheKey = md5($now.$streamId.json_encode($request->all()).json_encode(cstr_config()));
            ...
            if ($stream) {
                ...
                if (isset($path)) {
                    $query = [
                        ...
                        'metaType' => $metaType,
                        'userId' => $userId,
                        'apiKey' => cstr_config('api_key'),
                        'expires' => Carbon::now()->addMinutes(cstr_config('stream.stream_expires_ttl'))->timestamp,
                    ];
        ...
        if(isset($streamUrl)){
            Cache::put('stream_url_'.$streamCacheKey, $streamUrl, Carbon::now()->addMinutes(cstr_config('stream.stream_url_ttl')));
            Cache::put('stream_headers_'.$streamCacheKey, $streamHeaders, Carbon::now()->addMinutes(cstr_config('stream.stream_url_ttl')));

            //Media Flow Proxy
            if($mediaFlowProxy && cstr_config('mediaflowproxy.enabled')){
                $mfp = new MediaFlowProxyManager();
                $mfp->setUrl($streamUrl);

                if(cstr_config('mediaflowproxy.enabled_external') && !empty(cstr_config('mediaflowproxy.url')))
                    $mfp->useRemoteServer(cstr_config('mediaflowproxy.url'), cstr_config('mediaflowproxy.api_password'));
