...
class JellyfinMiddleware
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if(md5(trim($url)) !== md5(trim(cstr_config('url'))) && !str_contains(parse_url($url, PHP_URL_HOST), 'localhost')) {
            if(ping($url)) {
                cstr_config('url', $url);
                $libraryRebuild = true;
            }
        ...
        if(isset($header['authorization']) && isset($header['user-agent']) &&
            ...
            if(!empty($configuration) && isset($configuration['UICulture'])){
                if($configuration['UICulture'] !== cstr_config('lang'))
                    cstr_config('lang', $configuration['UICulture']);
            }
        }
