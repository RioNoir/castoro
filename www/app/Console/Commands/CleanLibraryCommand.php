...
class CleanLibraryCommand extends Command
{
    ...
    protected $description = 'Command to clean Castoro Library';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $items = Items::query()->whereNull('item_jellyfin_id')->whereNull('item_path')
                ->where('created_at', '<=', Carbon::now()->subDays(cstr_config('jellyfin.delete_unused_after')))->get();
            $count = $items->count();
            ...
            Streams::query()->where('created_at', '<=', Carbon::now()->subHours(cstr_config('jellyfin.delete_streams_after')))->delete();
