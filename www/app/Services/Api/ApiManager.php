...
class ApiManager
{
    ...
    protected function apiCall(string $uri, string $method = 'GET', array|string $data = [], array $headers = []) {
        ...
        if(!empty(cstr_config('http_proxy.host')) && cstr_config('http_proxy.enabled') && $host !== "localhost" && !local_ip($host) &&
            (empty(cstr_config('http_proxy.excluded_domains')) || !str_contains_arr($host, cstr_config('http_proxy.excluded_domains')))) {

            $options['proxy'] = "http://";
            if(!empty(cstr_config('http_proxy.username')) && !empty(cstr_config('http_proxy.password'))){
                $options['proxy'] .= cstr_config('http_proxy.username').":".cstr_config('http_proxy.password').'@';
            }
            $options['proxy'] .= cstr_config('http_proxy.host');
            if(!empty(cstr_config('http_proxy.port'))){
                $options['proxy'] .= ":".cstr_config('http_proxy.port');
            }
        }
