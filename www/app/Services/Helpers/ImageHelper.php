...
class ImageHelper
{
        ...
        try {
            ...
            if(isset($data['title'])){
                ...
                if(isset($data['catalog'])){
                    ...
                    $path = cstr_config('jellyfin.'.$type.'_path');
    ...
    public static function getImageByName(string $imageText, $fillHeight = null, $fillWidth = null, $imagesPath = null){
        ...
        if(file_exists(cstr_data_path('app/images/' . $imageId . '.png')))
            return cstr_data_path('app/images/' . $imageId . '.png');
        ...
        if(isset($image)){
            try {
                ...
                if (!file_exists(cstr_data_path('app/images')))
                    mkdir(cstr_data_path('app/images'), 0777, true);

                //Save
                $imageId = md5($imageText)."_w".$width."_h".$height;
                $imagePath = cstr_data_path('app/images/' . $imageId . '.png');
    ...
    public static function getRandomImageFromLibrary($imagesPath = null){
        if(!isset($imagesPath))
            $imagesPath = cstr_data_path('library/');

        $images = get_files_from_dir($imagesPath, ['jpg', 'jpeg', 'png']);

        if(count($images) <= 10)
            $images = get_files_from_dir(cstr_data_path('library/'), ['jpg', 'jpeg', 'png']);
