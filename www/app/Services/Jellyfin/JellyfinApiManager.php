...
class JellyfinApiManager extends ApiManager
{
    ...
    public function __construct(array $headers = []){
        $this->timeout = 30;
        $this->endpoint = cstr_config('jellyfin.url');
    ...
    public function getStreamingLanguageByUser(string $userId = null){
        $streamLang = cstr_config('stream.lang');
...
//        }
    ...
    public function deleteVirtualFolderIfNotPrimary(string $folderName = null){
        if(isset($folderName)) {
            ...
            if(!empty($virtualFolders)){
                ...
                if(in_array(cstr_config('jellyfin.movies_path'), $virtualFolder['Locations']) ||
                    in_array(cstr_config('jellyfin.series_path'), $virtualFolder['Locations']))
                    return [];
    ...
    public static function selfCall(Request $request, array $data = [], $returnBody = false){
        ...
        $url = cstr_config('jellyfin.url').$uri.'?'.http_build_query($request->query());
