...
class JellyfinItem
{
        ...
        if(isset($this->localItem)) {
            if($this->isMovie() || $this->isSeries()) {
                ...
                $this->item['ExternalUrls'][] = [
                    'Name' => 'Castoro',
                    'Url' => app_url('/web/#/configurationpage?name=CSTR_ITEM&itemId=' . @$this->localItem->item_md5.'&action=edit')
                ];
            }
            if($this->isMovie() || $this->isEpisode()){
                $this->item['ExternalUrls'][] = [
                    'Name' => 'Castoro Download',
                    'Url' => app_url('/web/#/configurationpage?name=CSTR_ITEM&itemId=' . @$this->localItem->item_md5. '&jItemId='.$this->itemId.'&action=download')
                ];
            }
        }
    ...
    public function updateStrm(){
        ...
        if(isset($strmFile)){
            ...
            $sourceQuery['apiKey'] = cstr_config('api_key');
...
//            }
            ...
            if(isset($outcomeItem['Id'])) {
                ...
                if(!isset($item)) {
                    ...
                    $item->item_server_id = cstr_config('server_id');


                    if(!in_array(cstr_data_path($path), [
                        cstr_config('jellyfin.movies_path'),
                        cstr_config('jellyfin.series_path'),
                        cstr_config('jellyfin.tv_path'),
                    ])) {
                        $item->item_path = $path;
                        $item->save();
                    }
    ...
    public function getMediaSources(){
        try {
            ...
            if (isset($metaId)) {
                ...
                $key = md5($now. json_encode($mediaSources) . $itemId . $metaId . $metaType . $mediaSourceId . $userId. json_encode(cstr_config()));
                        ...
                        foreach ($mediaSources as $key => &$mediaSource) {
                            if(!isset($mediaSource['SourceType']) || $mediaSource['SourceType'] != 'ExternalStream') {
                                ...
                                if (($mediaSource['Protocol'] == "Http" && $mediaSource['Id'] == $this->item['Id']) ||
                                    ...
                                    $mediaSource['Name'] = "[HTTP] " . cstr_trans('stream_auto');
                                    $mediaSource['SourceType'] = 'ExternalStream';
                                }
